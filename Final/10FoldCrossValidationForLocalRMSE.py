...
X_sub = X_sub.drop(overfit, axis=1) # X_sub is not used, but we keep it to match columns
...
# --- 5. LOCAL 10-FOLD CV EVALUATION ---
print("\nStarting 10-fold cross-validation...")

def rmsle_cv(model, X_data=X, y_data=y):
    """
    Calculates the Root Mean Squared Logarithmic Error (RMSLE) using 10-fold CV.
    Since our target 'y' is already log-transformed, we just need to calculate
    the regular RMSE on it, which is equivalent to RMSLE on the original data.
    """
    # We pass the kfolds object defined in Section 4
    # We use 'neg_mean_squared_error' as score, then take its sqrt and negative
    rmse = np.sqrt(-cross_val_score(model, X_data.values, y_data.values, 
...
# print(f"  Lasso RMSE: {score_lasso.mean():.6f} (std: {score_lasso.std():.6f})")

# score_ridge = rmsle_cv(ridge)
# print(f"  Ridge RMSE: {score_ridge.mean():.6f} (std: {score_ridge.std():.6f})")

# score_gbr = rmsle_cv(gbr)
# print(f"  GBR RMSE:   {score_gbr.mean():.6f} (std: {score_gbu.std():.6f})")
# print("...") # Add more models here if you want to test them individually

# --- Evaluate Stacking Models ---
print("Evaluating Stacking Models...")

score_xgb_stack = rmsle_cv(stack_gen_xgb)
print(f"  Stack (XGB Meta) RMSE: {score_xgb_stack.mean():.6f} (std: {score_xgb_stack.std():.6f})\n")

score_lasso_stack = rmsle_cv(stack_gen_lasso)
print(f"  Stack (Lasso Meta) RMSE: {score_lasso_stack.mean():.6f} (std: {score_lasso_stack.std():.6f})\n")

print("--- CV Evaluation Finished ---")
print(f"\nLocal RMSE for Stack (XGB Meta):   {score_xgb_stack.mean():.6f}")
print(f"Local RMSE for Stack (Lasso Meta): {score_lasso_stack.mean():.6f}")